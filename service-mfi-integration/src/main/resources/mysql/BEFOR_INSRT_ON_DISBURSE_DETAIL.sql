DROP TRIGGER IF EXISTS BEFOR_INSRT_ON_DISBURSE_DETAIL;
/
CREATE
 TRIGGER BEFOR_INSRT_ON_DISBURSE_DETAIL BEFORE INSERT ON disbursement_detail  FOR EACH ROW BEGIN  
	   ...
	   DECLARE isalreadysanction BIGINT(20);
	    ...
	    ELSEIF (sanctionamount < disburseamt)THEN
	       SET res = 14;
	    END IF;
	 UPDATE bank_current_proposal_status SET bank_curr_proposal_status_id=res,is_saved=new.is_saved, modified_date=NOW() , disbursed_pk = new.id ,
	  disbursed_date = new.disbursement_date  WHERE application_id = new.application_id ;
